...
class ProfileApiTest : public InitializedApiTest {
   protected:
    void ExpectCurrentProfile(const char* expected_id) {
        ...
        ASSERT_EQ(ox_sim_get_profile(profile_name, sizeof(profile_name)), OX_SIM_SUCCESS);
...
class DeviceApiTest : public InitializedApiTest {
   protected:
    const ox_sim::DeviceProfile* SetProfileAndGet(const char* profile_id) {
        const OxSimResult result = ox_sim_set_profile(profile_id);
        if (result != OX_SIM_SUCCESS) {
            ADD_FAILURE() << "ox_sim_set_profile failed for " << profile_id << ": " << result;
            return nullptr;
        }
...
class InputApiTest : public InitializedApiTest {
   protected:
    void SetQuest2Profile() { ASSERT_EQ(ox_sim_set_profile("oculus_quest_2"), OX_SIM_SUCCESS); }

    void ExpectFloat(const char* user_path, const char* component_path, float expected) {
        float value = 0.0f;
        ASSERT_EQ(ox_sim_get_input_float(user_path, component_path, &value), OX_SIM_SUCCESS);
        EXPECT_FLOAT_EQ(value, expected);
    }

    void ExpectBoolean(const char* user_path, const char* component_path, XrBool32 expected) {
        XrBool32 value = XR_FALSE;
        ASSERT_EQ(ox_sim_get_input_boolean(user_path, component_path, &value), OX_SIM_SUCCESS);
        EXPECT_EQ(value, expected);
    }

    void ExpectVec2(const char* user_path, const char* component_path, float expected_x, float expected_y) {
        XrVector2f value = {};
        ASSERT_EQ(ox_sim_get_input_vector2f(user_path, component_path, &value), OX_SIM_SUCCESS);
