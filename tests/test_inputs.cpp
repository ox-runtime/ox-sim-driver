...
TEST_F(InputApiTest, RoundTripsBooleanInput) {
    SetQuest2Profile();
    ASSERT_EQ(ox_sim_set_input_boolean("/user/hand/left", "/input/x/click", XR_TRUE), OX_SIM_SUCCESS);
    ExpectBoolean("/user/hand/left", "/input/x/click", XR_TRUE);
}

TEST_F(InputApiTest, RoundTripsFloatInput) {
    SetQuest2Profile();
    ASSERT_EQ(ox_sim_set_input_float("/user/hand/left", "/input/trigger/value", 0.75f), OX_SIM_SUCCESS);
...
TEST_F(InputApiTest, RoundTripsVectorInput) {
    ...
    ASSERT_EQ(ox_sim_set_input_vector2f("/user/hand/left", "/input/thumbstick", &vec_value), OX_SIM_SUCCESS);
...
TEST_F(InputApiTest, SyncsVectorFromLinkedAxis) {
    ...
    XrVector2f vec_value = {0.25f, -0.5f};
    ASSERT_EQ(ox_sim_set_input_vector2f("/user/hand/left", "/input/thumbstick", &vec_value), OX_SIM_SUCCESS);
    ASSERT_EQ(ox_sim_set_input_float("/user/hand/left", "/input/thumbstick/x", -0.75f), OX_SIM_SUCCESS);
...
TEST_F(InputApiTest, RejectsUnknownComponentPath) {
    ...
    EXPECT_EQ(ox_sim_get_input_float("/user/hand/left", "/input/does_not_exist", &axis_value),
